<%@ WebHandler Language="VB" Class="Art_Say" %>

Imports System
Imports System.Web
Imports System.Data.OleDb                                                          '引入Access数据库相关的命名空间

Public Class Art_Say : Implements IHttpHandler : Implements IRequiresSessionState
        ...
        If context.Request.QueryString("mode") = "post" Then
            Select Case context.Request.Form("do")
                ...
                Case "new"
                    ...
                    CMD.Connection = CN
                    CMD.CommandText = "INSERT INTO [Art_Say] (IsLogin, Body, UName,IP,AID) VALUES(@IsLogin, @Body, @UName,@IP,@AID)"
                    '向SQL语句中补充数据
                ...
                Case "get"
                    ...
                    Dim CMD As New OleDbCommand
                    CMD.Connection = CN
                    CMD.CommandText = "SELECT * FROM [Art_Say] WHERE [AID]=@AID ORDER BY [TheTime] DESC"
                    CMD.Parameters.AddWithValue("@AID", context.Request.Form("aid"))
